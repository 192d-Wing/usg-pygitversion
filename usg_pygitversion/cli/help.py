...
# ruff: noqa: E501 -- vendored upstream text keeps its line lengths
"""Help text. The first block is vendored from upstream ``docs/usage/cli/arguments.md`` (6.8.2)."""
...
ADDITIONS = """\
# usg-pygitversion additions (not in the .NET tool)

    /output env     Prints shell assignments (export GitVersion_<Name>=...)
                    that are safe to eval; suppresses the JSON when it is the
                    only output. See /shell and /prefix.
    /shell          Shell dialect for /output env: sh (default), bash,
                    powershell or cmd.
    /prefix         Environment variable prefix for /output env and
                    /envfile (default GitVersion_).
    /envfile        Appends GitVersion_<Name>=<value> lines to this file, the
                    contract used by $GITHUB_ENV and similar CI mechanisms.
    --<name>        Every switch also accepts the POSIX double-dash form.
"""


def help_text(version: str) -> str:
    """Ports ``HelpWriter.WriteTo``: banner, blank line, argument list."""
    return f"GitVersion {version}\n\n{UPSTREAM_ARGUMENTS}\n\n{ADDITIONS}"
