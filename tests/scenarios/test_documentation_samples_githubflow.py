...
"""Port of upstream ``DocumentationSamplesForGitHubFlow.cs`` (6.8.2)."""
...
@pytest.mark.parametrize("with_pull_request", [False, True])
def test_feature_branch_with_mainline(with_pull_request: bool) -> None:
    configuration = githubflow(
        next_version="1.2.0", strategies=["ConfiguredNextVersion", "Mainline"]
    )
    with Scenario() as f:
        f.make_a_commit()
