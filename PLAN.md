...
**Phase 6: Hardening and release (1 wk)** — in progress: real-repo and 50k-commit checks done, CI budget/integration jobs added; release tag pending
- Run against real repositories and diff against the local `gitversion`
  6.8.2 output (done on three local repositories, all identical); CI runs
  the same comparison on every ported scenario with the .NET tool installed.
- Performance check on a large repo (goal: < 2 s for 50k commits; measured
  0.9 s and +28 MB, enforced by the `budget` CI job).
- README, deviations doc, `uv tool install` instructions.
- Tag `v0.1.0` for the first public release (`v1.0.0` once the API has been used in anger); release.yml publishes the wheel to PyPI and creates the
  GitHub release.
